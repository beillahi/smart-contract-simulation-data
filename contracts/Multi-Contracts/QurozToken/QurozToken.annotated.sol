/* 
 source code generate by Bui Dinh Ngoc aka ngocbd<buidinhngoc.aiti@gmail.com> for smartcontract QurozToken at 0x90c93e7fe79fd0c9d1a437ac715ff8752e3eac88
*/

// Link to source contract
// https://github.com/ngocbd/smartcontract/blob/master/QurozToken.sol
...
/// @notice simulation __verifier_eq(QurozToken$paused, Pausable_spec$paused)
contract QurozToken {
  ...
  uint256 public totalSupply_ = 12e8 * 1000000000000000000;             //BasicToken.sol
  uint256 public cap = 12e8 * 1000000000000000000;               //CappedToken.sol
...
//
  ...
  */
  ...
  //function destroyAndSend(address payable _recipient) onlyOwner public {   //Destructible.sol
  //  selfdestruct(_recipient);
  //}
  
/* new functions */
  function startMinting(string memory reason) onlyOwner cannotMint public returns (bool) {
  ...
  function setCap(uint256 _cap, string memory _reason) public onlyOwner {
    _setCap(_cap, _reason);
  }

  function _setCap(uint256 _cap, string  memory _reason) internal onlyOwner { // change the limit of cap
    cap = _cap;
    emit CapChange(_cap, _reason);
  }
  
  function multiTransfer(address[] memory _to, uint256[] memory _amount) whenNotPaused public returns (bool) {
  ...
  function multiMint(address[] memory _to, uint256[] memory _amount, string memory _reason) hasMintPermission canMint public returns (bool) {
    require(_to.length == _amount.length);
  ...
  function multiLock(address[] memory _lockAddress) public onlyOwner {
    for (uint i=0; i < _lockAddress.length; i++){
      lock(_lockAddress[i]);
    }
  }

  function multiUnlock(address[] memory _lockAddress) public onlyOwner {
    for (uint i=0; i < _lockAddress.length; i++){
