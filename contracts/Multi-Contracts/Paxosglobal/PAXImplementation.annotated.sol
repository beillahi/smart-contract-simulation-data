...
// https://github.com/paxosglobal/pax-contracts/blob/master/contracts/PAXImplementation.sol
 ...
 *
 * @notice simulation PAXImplementation$paused == Pausable_spec$paused && PAXImplementation$owner == Pausable_spec$_owner
 * @notice simulation PAXImplementation$owner == Ownable$_owner
 * @notice simulation __verifier_eq(PAXImplementation$balances, ERC20_spec$balances) && __verifier_eq(PAXImplementation$allowed, ERC20_spec$allowances)
 */
/// @notice simulation __verifier_eq(PAXImplementation$balances, ERC20_spec$balances) && __verifier_eq(PAXImplementation$allowed, ERC20_spec$allowances)
contract PAXImplementation {
    ...
    /**
        ...
        @notice modifies totalSupply_
        @notice modifies supplyController
        @notice modifies lawEnforcementRole
        @notice modifies initialized
   */
   ...
   */
    ...
    * @param val The amount to be transferred.
    ...
    */
     ...
     * @param val uint256 the amount of tokens to be transferred
     * @notice modifies balances[from]
     * @notice modifies balances[to]
     * @notice modifies allowed[from][msg.sender]
     */
    function transferFrom(
        address from, address to, uint val
    )
    public
    whenNotPaused
    //returns (bool)
     ...
     * @param val The amount of tokens to be spent.
